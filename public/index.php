...
function friendshipActionMarkup(chatUser) {
    ...
    if (status === 'pending') {
        return `<button class="mini-button danger icon-button" type="button" data-request-action="cancel_friend_request" data-user-id="${userId}" aria-label="Cancel friend request" title="Cancel friend request">${rejectIcon}</button>`;
    }
