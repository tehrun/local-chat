...
Then open <http://127.0.0.1:8000>. When running on PHP's built-in dev server from a loopback host such as `127.0.0.1` or `localhost`, the chat UI falls back to short polling instead of Server-Sent Events so message sends do not get blocked by the single-worker server. On non-loopback hosts, the conversation page keeps a single SSE connection open and now checks a lightweight conversation signature before reloading the full payload, which cuts down repeated message reads against the database. On phones, install the app behind HTTPS or use a secure local tunnel so the browser can grant microphone access for hold-to-record voice notes.
