...
Then open <http://127.0.0.1:8000>. When running on PHP's built-in dev server, the chat UI falls back to short polling instead of Server-Sent Events so message sends do not get blocked by the single-worker server. That polling path now checks a lightweight conversation signature first and only fetches the full `messages` payload when something actually changed, which avoids hammering the full conversation query on every poll. On multi-worker hosting, the conversation page keeps a single SSE connection open and also uses the lightweight signature to avoid unnecessary full payload reloads. On phones, install the app behind HTTPS or use a secure local tunnel so the browser can grant microphone access for hold-to-record voice notes.
